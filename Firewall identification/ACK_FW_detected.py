...
if (ACK_response == None) and (SYN_response == None): 
 #Si no se recibe respuesta en ninguno de los  dos paquetes, es muy probable que el host este dowm
 ...
 #En caso de que uno y solo uno de los  paquetes enviados haya recibido respuesta
 ...
 #Si el valor del flag en al cabecera TCP es de 18, es decir SYN ACK, significa que el host respondio exitosamente, confirmando la sincronizacion de una conexion
 print "Port is unfiltered and open"     
 #por lo tanto consideramos que el puerto se encuentra abierto
elif int(SYN_response[TCP].flags) == 20: 
 #Si el valor del flag en la cabecera TCP es de 20, es decir SYN RST, signiifica que el host respondio exitosamente, pero no quiere establecer una conexion
 print "Port is unfiltered and closed"   
 #por lo tanto consideramos que el puerto se encuentra cerrado
else:
 print "Unable to determine if the port is filtered" 
 #Si nada d elo anterior fue confirmado, entonces no se puede  determinar si el puerto esta filtrado
